...
def get_unitsTickerRnd(tickerIdx, pTicker, cash, max_trials=50000):
    """
    Find the integer allocation of tickers that maximizes total value < cash.
    """
    tickers = pd.Index(tickerIdx)

    # If even the cheapest share costs more than cash → impossible
    if pTicker.min() >= cash:
        return None

    # Upper bound for each ticker = floor(cash / price)
    max_units = (cash // pTicker).astype(int)

    best_value = -np.inf
    best_rnd = None

    # Try all combinations (only feasible for small number of tickers!)
    for combo in itertools.product(*(range(m+1) for m in max_units)):
        rnd = pd.Series(combo, index=tickers)
        value = (rnd * pTicker).sum()
        if value < cash and value > best_value:
            best_value = value
            best_rnd = rnd

    return best_rnd if best_rnd is not None else None
