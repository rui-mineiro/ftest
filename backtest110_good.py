...
S_K , C_K = 5/100 , 10 # threshold and cooldown
SA = 0
SB = 0
C = 0
...
for t, (date, row) in enumerate(data.iterrows(), start=1):
    ...
    if C > 0:
        ...
        if SA >= S_K and unitsB>1 and value >= pA and unitsB0>0:
            unitsB  -= unitsB0
            cash    += pB*unitsB0
            unitsA0  = cash // pA
            unitsA  += unitsA0
            cash    -= unitsA0*pA
            moved = f"{unitsA0}{tickerB}→{tickerA}"
        elif SB >= S_K and unitsA>1 and value >= pB  and unitsA0>0:
            unitsA  -= unitsA0
            cash    += pA*unitsA0
            unitsB0  = cash // pB
            unitsB  += unitsB0
...
fig.add_trace(go.Scatter(
    x=df["date"], y=df["SA"]*1000,
    mode="lines", name="ScoreA",
    line=dict(color="green", dash="dot")
), secondary_y=True)

fig.add_trace(go.Scatter(
    x=df["date"], y=df["SB"]*1000,
    mode="lines", name="ScoreB",
    line=dict(color="green", dash="dot")
), secondary_y=True)
