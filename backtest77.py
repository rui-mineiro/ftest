...
def get_price_next(symbol,startdate,enddate):
    ...
    price = vbt.YFData.download(symbol,start=startdate,end=enddate).get('Close')
    n=1
    price_next=vbt.YFData.download(symbol,start=startdate,end=enddate+timedelta(days=n)).get('Open')
    while price_next.index[-2]!=price.index[-1]:
        n+=1
        price_next=vbt.YFData.download(symbol,start=startdate,end=enddate+timedelta(days=n)).get('Open')
...
def get_entries_exits(price_next,entries,exits,mas):
