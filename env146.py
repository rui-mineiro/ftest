...
indicators = ["MA05", "MA10", "MSTD05", "MSTD10", "EMA05", "EMA10" , "PCT01" , "PCT05" , "PCT10" , "TRMA05", "TRSTD10" ]
...
def get_indicator(data: pd.DataFrame, indicators: list[str], fields=None) -> pd.DataFrame:
    ...
    # ----- True Range rolling stats (keys: TRMA{w}, TRSTD{w}) -----
        ...
        for ticker in H.columns.intersection(L.columns).intersection(C.columns):
            ...
            for ind in indicators:
                ...
                if ind.startswith("TRMA"):
                    ...
                    vals = tr.rolling(w).mean().fillna(0)
                    cols[("TR", f"MA{w}", ticker)] = vals
                elif ind.startswith("TRSTD"):
                    if not w: raise ValueError("TRSTD requires a window, e.g., TRSTD14")
                    vals = tr.rolling(w).std(ddof=0).fillna(0)
                    cols[("TR", f"MSTD{w}", ticker)] = vals
                # ignore other indicators here

    # ----- Per-field indicators (MA, MSTD, EMA, PCT) -----
    for price_field in fields:
        ...
        for ticker in df_field.columns:
            ...
            for ind in indicators:
                ...
                if ind.startswith("MA") and not ind.startswith("TRMA"):
                    ...
                    continue  # handled earlier or not applicable to this price_field
...
def get_indicator2(data: pd.DataFrame, indicators: list[str], fields=None) -> pd.DataFrame:
