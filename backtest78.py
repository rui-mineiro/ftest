...
def get_strategy_signals(price_next, mas):
    """
    ...
    return entries, exits, b_fast_ma, b_slow_ma


def plot_strategy(symbol, price_next, entries, exits, b_fast_ma, b_slow_ma, b_f, b_s, pf_best, signal_text):
    """
...
symbols = ['SPPW.DE', 'DAVV.DE']  # podes meter quantos quiseres
...
for symbol in symbols:
    ...
    entries, exits, b_fast_ma, b_slow_ma = get_strategy_signals(price_next, mas)

    # 4. Backtest com parâmetros ótimos
    # raw_entries_best = (b_fast_ma.iloc[:, 0] > b_slow_ma.iloc[:, 0])
    raw_entries_best = (b_fast_ma > b_slow_ma)

    raw_exits_best = (vbt.MA.run(price_next, window=int(s_f)).ma <
                      vbt.MA.run(price_next, window=int(s_s)).ma)
    entries_best, exits_best = raw_entries_best.vbt.signals.clean(raw_exits_best)
    pf_best = vbt.Portfolio.from_signals(price_next, entries_best, exits_best, init_cash=10000, fees=0.001)
    ...
    # 6. Gráfico
    plot_strategy(symbol, price_next, entries, exits, b_fast_ma, b_slow_ma, b_f, b_s, pf_best, signal_text)
